...
1. From the root of the repo, `cd` into kubeflow's  `manifests` directory and make sure you are in the `v1.8-branch`.
...
1. Configure TLS. Start by getting IP address of istio gateway
    ...
    Replace the IP address in the tls/certificate.yaml file (line 13) with the IP address of the istio gateway and save the file.
